...
{
    auto wordStatis = wordsStatis.find(word);
    if (wordStatis == wordsStatis.end())
    {
        // 如果当前统计表中没找到，则新增一项
        WordInfo *wordInfo = new WordInfo;
        wordInfo->lines.push_back(lineNo);
        wordsStatis[std::move(word)] = wordInfo;
    }
    else
    {
        // 如果找到了，则增加一个新的行号
        wordStatis->second->lines.push_back(lineNo);
    }
}

// 排序所需的比较函数
bool cmp(const WordPair &a, const WordPair &b)
{
