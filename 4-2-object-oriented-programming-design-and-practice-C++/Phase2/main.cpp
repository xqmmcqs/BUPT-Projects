...
int main()
{
    ...
    QVector<QString> status{"未支付", "已取消", "已支付", "退款中", "已退款"};
    ...
    while (true)
    {
        ...
        else if (list[0] == "DISPCART")
        {
            ...
            if (list.size() != 1) goto error;
            QJsonArray response;
            auto result = userManage.viewCart(token.toObject(), response);
            if (result.isEmpty())
            {
                auto total = response.size();
                qInfo() << "查询到" << total << "件商品";
        ...
        else if (list[0] == "ADDCART")
        {
            ...
            bool ok = false;
            if (list.size() != 3 || list[1].toInt(&ok), !ok || list[2].toInt(&ok), !ok) goto error;
            auto result = userManage.addToCart(token.toObject(), list[1].toInt(), list[2].toInt());
            if (result.isEmpty())
                qInfo() << "添加到购物车成功";
            else
                qInfo() << "添加到购物车失败" << result;
        }
        else if (list[0] == "SETCART")
        {
            if (token.isNull())
            {
                qInfo() << "未登录";
            ...
            bool ok = false;
            if (list.size() != 3 || list[1].toInt(&ok), !ok || list[2].toInt(&ok), !ok) goto error;
            auto result = userManage.modifyCart(token.toObject(), list[1].toInt(), list[2].toInt());
            if (result.isEmpty())
                qInfo() << "修改购物车内商品数量成功";
            else
                qInfo() << "修改购物车内商品数量失败" << result;
        }
        else if (list[0] == "REMOVECART")
        {
            if (token.isNull())
            {
                qInfo() << "未登录";
            ...
            bool ok = false;
            if (list.size() != 2 || list[1].toInt(&ok), !ok) goto error;
            auto result = userManage.removeFromCart(token.toObject(), list[1].toInt());
            if (result.isEmpty())
                qInfo() << "从购物车内移除商品成功";
            else
                qInfo() << "从购物车内移除商品失败" << result;
        }
        else if (list[0] == "ORDER")
        {
            ...
            {
                if (list[i].toInt(&ok), !ok)goto error;
                query.append(list[i].toInt());
            }
            auto result = userManage.generateOrder(token.toObject(), query);
            if (result.isEmpty())
                qInfo() << "下单成功";
            else
                qInfo() << "下单失败" << result;
        }
        else if (list[0] == "DISPMYORDER")
        {
            if (token.isNull())
            {
                qInfo() << "未登录";
            ...
            if (list.size() != 1) goto error;
            QJsonArray response;
            auto result = userManage.viewOrderByName(token.toObject(), response);
            if (result.isEmpty())
            {
                ...
                {
                    QJsonObject item = response[i].toObject();
                    qInfo() << "订单ID" << item["id"].toInt() << "金额" << item["total"].toDouble();
                }
            }
            else
                qInfo() << "查询失败" << result;
        }
        else if (list[0] == "DISPORDER")
        {
            if (token.isNull())
            {
                qInfo() << "未登录";
            ...
            if (list.size() != 2 || list[1].toInt(&ok), !ok) goto error;
            QJsonObject response;
            auto result = userManage.viewOrderDetails(token.toObject(), list[1].toInt(), response);
            if (result.isEmpty())
            {
                qInfo() << "主订单ID" << response["id"].toInt() << "主订单金额" << response["total"].toInt() / 100.0
                        << "主订单状态" << status[response["status"].toInt()];
                int total = response["order"].toArray().size();
                qInfo() << "查询到" << total << "个子订单";
                QJsonArray array = response["order"].toArray();
                for (int i = 0; i < total; ++i)
                {
                    QJsonObject item = array[i].toObject();
                    qInfo() << "订单ID" << item["id"].toInt() << "主订单ID" << item["mainid"].toInt() << "状态"
                            << status[item["status"].toInt()] << "金额" << item["total"].toInt() / 100.0 << "买家"
                            << item["buyer"].toString() << "卖家" << item["seller"].toString();
                }
                total = response["commodity"].toArray().size();
                qInfo() << "查询到" << total << "个商品";
                array = response["commodity"].toArray();
                for (int i = 0; i < total; ++i)
                {
                    QJsonObject item = array[i].toObject();
                    qInfo() << "商品ID" << item["id"].toInt() << "名称" << item["name"].toString() << "类型"
                            << commodityType[item["type"].toInt()] << "数量" << item["amount"].toInt() << "所属子订单ID"
                            << item["order"].toInt();
                }
            }
            else
        ...
        else if (list[0] == "CANCEL")
        {
            ...
            bool ok = false;
            if (list.size() != 2 || list[1].toInt(&ok), !ok) goto error;
            auto result = userManage.cancelOrder(token.toObject(), list[1].toInt());
            if (result.isEmpty())
                qInfo() << "取消订单成功";
            else
                qInfo() << "取消订单失败" << result;
        ...
        else if (list[0] == "PAY")
        {
            ...
            bool ok = false;
            if (list.size() != 2 || list[1].toInt(&ok), !ok) goto error;
            auto result = userManage.payOrder(token.toObject(), list[1].toInt());
            if (result.isEmpty())
                qInfo() << "付款成功";
            else
                qInfo() << "付款失败" << result;
        }
        else if (list[0] == "REFUND")
        {
            if (token.isNull())
            {
                qInfo() << "未登录";
            ...
            bool ok = false;
            if (list.size() != 2 || list[1].toInt(&ok), !ok) goto error;
            auto result = userManage.refundOrder(token.toObject(), list[1].toInt());
            if (result.isEmpty())
                qInfo() << "退款成功";
            else
                qInfo() << "退款失败" << result;
        ...
        else if (list[0] == "HELP")
        {
            ...
            if (list.size() == 1 || list[1].toUpper() == "DISPCART")
            {
                qInfo() << "DISPCART";
                qInfo() << "    查看购物车中的商品";
            }
            if (list.size() == 1 || list[1].toUpper() == "ADDCART")
            {
                qInfo() << "ADDCART <商品ID> <数量>";
                qInfo() << "    向购物车中添加商品";
            }
            if (list.size() == 1 || list[1].toUpper() == "SETCART")
            {
                qInfo() << "SETCART <商品ID> <数量>";
                qInfo() << "    修改购物车中某个商品的数量";
            }
            if (list.size() == 1 || list[1].toUpper() == "REMOVECART")
            {
                qInfo() << "REMOVECART <商品ID>";
                qInfo() << "    从购物车中移除某个商品";
            }
            if (list.size() == 1 || list[1].toUpper() == "ORDER")
            {
                qInfo() << "ORDER <商品ID> <商品ID> ...";
                qInfo() << "    将购物车中的部分商品下单";
            }
            if (list.size() == 1 || list[1].toUpper() == "DISPMYORDER")
            {
                qInfo() << "DISPMYORDER";
                qInfo() << "    查看自己的订单";
            }
            if (list.size() == 1 || list[1].toUpper() == "DISPORDER")
            {
                qInfo() << "DISPORDER <订单ID>";
                qInfo() << "    查看某个订单的详情";
            }
            if (list.size() == 1 || list[1].toUpper() == "CANCEL")
            {
                qInfo() << "CANCEL <订单ID>";
                qInfo() << "    取消某个订单";
            }
            if (list.size() == 1 || list[1].toUpper() == "PAY")
            {
                qInfo() << "PAY <订单ID>";
                qInfo() << "    付款某个订单";
            }
            if (list.size() == 1 || list[1].toUpper() == "REFUND")
            {
                qInfo() << "REFUND <订单ID>";
                qInfo() << "    退款某个订单";
            }
