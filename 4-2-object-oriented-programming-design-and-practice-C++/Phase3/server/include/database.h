 ...
 *
 * 对于订单部分，定义了插入订单、查询订单、修改订单状态的接口。
 *
 * 除此之外，定义了几个工具函数的接口，例如打印调试信息，以及将商品或订单的查询结果转化为商品或订单类。
*/

#ifndef PHASE3_DATABASE_H
#define PHASE3_DATABASE_H
...
private:
    ...
    /**
     * @brief 将订单查询结果转化为订单类
     * @param sqlQuery 订单的查询结果
     * @return 指向一个新创建的订单类的指针
     */
    static QSharedPointer<Order> orderResult2Object(const QSqlQuery &sqlQuery);
...
public:
    ...
    void insertOrderData(int id, int mainId, int total, const QString &buyer, const QString &seller,
                         const QString &content) const;
    
    bool queryOrderById(int id, QSharedPointer<Order> &result) const;
    
    int queryOrderByMainId(int mainId, QList<QSharedPointer<Order>> &result) const;
    
    int queryOrderByBuyerName(const QString &name, QList<QPair<int, int>> &result) const;
    
    int queryOrderBySellerName(const QString &name, QList<QPair<int, int>> &result) const;
    
    void modifyOrderStatus(int id, int status) const;
};

#endif //PHASE3_DATABASE_H
